...
void main() {
  test('dioClientProvider builds a Dio with the three interceptors attached, '
  ...
  test(
    'dioClientProvider itself is overridable in a throwaway test container',
    () {
  ...
  test(
    'authTokenGetterProvider defaults to a no-op getter returning null',
    () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
  ...
  test('RefreshInterceptor (P-022A) is wired in after ErrorInterceptor, '
    ...
    final refreshIndex = dio.interceptors.indexWhere(
      (i) => i is RefreshInterceptor,
    );

    expect(refreshIndex, isNot(-1), reason: 'RefreshInterceptor missing');
    expect(errorIndex, lessThan(refreshIndex));
  });
}
